...
my %anonymized_ip;	
my %anonymized_text;
my $ano_txt = "AnonAAAA";	# starting pattern - needs to be alpha chars only for incrementing to work
my $ano_suffix = '.anon';

sub remove_keys {
	my $line = shift;
	my @pattern_list = ();
	my $replacement = '\'removed by cactus-anonymizer\'';
	# fortinet:
